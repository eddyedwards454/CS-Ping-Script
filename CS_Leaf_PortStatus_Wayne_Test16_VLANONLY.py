...
def main():
    if os.path.exists('C://SCRIPTS//SCRIPTS//Python Scripts//Learning Home Work//ACI Inventory//ACI_Node_Output16.csv'):
        os.remove('C://SCRIPTS//SCRIPTS//Python Scripts//Learning Home Work//ACI Inventory//ACI_Node_Output16.csv')
    ...
    x = open ('C://SCRIPTS//SCRIPTS//Python Scripts//Learning Home Work//ACI Inventory//ACI_Node_Output16.csv',
    'w')
    x.writelines('APIC,Port-Node,Switch State,Admin State,Access VLAN,Internal VLAN ' + '\n')
    x.close()
    ...
    for aci in aci_devices:
        ...
        # for Pod in (1):
            ...
            #### TEST to get Vlan#######
                with open(
                        'C://SCRIPTS//SCRIPTS//Python Scripts//Learning Home Work//ACI Inventory//ACI_Node_Output16.csv',
                        'a') as csv_file:
                    # csv_file.writelines('APIC,Port-Node,Switch State,Admin State,Access VLAN,Internal VLAN ' + '\n')
                    for item1 in (fabric_id3):
                        # initial_data1 = item1["l1PhysIf"]["attributes"]["dn"] + ',' + item1["l1PhysIf"]["attributes"][
                        #     "switchingSt"] + ',' + item1["l1PhysIf"]["attributes"]["adminSt"]
                        # initial_data2 = item2["ethpmPhysIf"]["attributes"]["accessVlan"] + "," + item3["vlanCktEp"]["attributes"]["encap"] +'\n'
                        initial_data3 = ACI_node + item1["vlanCktEp"]["attributes"]["encap"] +'\n'
                        # initial_data2 = item2["ethpmPhysIf"]["attributes"]["allowedVlans"] + '\n'
                        # csv_line = aci + ', ' + initial_data1 + ',' + initial_data2
                        csv_line = initial_data3
                        # print(csv_line)
                        csv_file.writelines(csv_line)
